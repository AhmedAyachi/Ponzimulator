...
pub struct CashFlow {} impl CashFlow {
    ...
    pub fn getPotentialEarnings(accounts:&Vec<&mut Account>)->f64{
    ...
    pub fn getNetPot(accounts:&Vec<&mut Account>)->f64{
        let totalPot=CashFlow::getTotalPot(accounts);
        let currentPot=CashFlow::getCurrentPot(accounts);
        return totalPot-currentPot;
    }

    pub fn getTotalPot(accounts:&Vec<&mut Account>)->f64{
        return accounts.iter().fold(0.0,|sum,account|{
            return sum+account.pot;
        });
    }

    pub fn getCurrentPot(accounts:&Vec<&mut Account>)->f64{
        return accounts.iter().fold(0.0,|sum,account|{
            return sum+account.balance;
        });
    }

    pub fn getTotalDeposit(accounts:&Vec<&mut Account>)->f64{
        return accounts.iter().fold(0.0,|sum,account|{
